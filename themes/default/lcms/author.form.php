...
<form action="<?php echo $this->url('lcms', "edit") ?>" method="post" class="generic-form">
    <table class="generic-form-table">
        <tr>
            ...
            <td>
                <select name="account_id" id="account_id" <?php echo $readonly ?>>
                    ...
                    <option value="<?php echo $account->account_id ?>" <?php echo $disabled ?> <?php echo ($content_res->account_id == $account->account_id) ? "selected" : "" ?>><?php echo $account->userid ?></option>
                    <?php endforeach; ?>
                    <?php endif; ?>
                </select>
            </td>
        ...
        <tr>
            <th></th>
            <td>
                <button title='<?php echo Flux::message('LcmsN' . ucfirst($tsk)) ?> <?php echo Flux::message('LcmsTypeAuthor') ?>' name='tsk' value='author;do<?php echo $tsk ?>;<?php echo htmlspecialchars($content_res->account_id) ?>' style='background:none;border:none;cursor:pointer'>
